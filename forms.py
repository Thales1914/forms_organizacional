...
def formulario():
    ...
    with st.form("pesquisa", clear_on_submit=True):
        ...
        respostas["p3"] = st.radio("3) Relação com a equipe", list(PESOS["p3"].keys()), horizontal=True, index=list(PESOS["p3"].keys()).index(st.session_state["respostas"].get("p3", list(PESOS["p3"].keys())[0])))
        respostas["j3"] = st.text_area("Justifique (Q3)", value=st.session_state["respostas"].get("j3", ""))

        progresso += 1
        st.progress(progresso / total_perguntas)

        respostas["p4"] = st.radio("4) Sua relação com o colega", list(PESOS["p4"].keys()), horizontal=True, index=list(PESOS["p4"].keys()).index(st.session_state["respostas"].get("p4", list(PESOS["p4"].keys())[0])))
        respostas["j4"] = st.text_area("Justifique (Q4)", value=st.session_state["respostas"].get("j4", ""))

        progresso += 1
        st.progress(progresso / total_perguntas)

        respostas["p5"] = st.radio("5) Colabora com a equipe?", list(PESOS["p5"].keys()), horizontal=True, index=list(PESOS["p5"].keys()).index(st.session_state["respostas"].get("p5", list(PESOS["p5"].keys())[0])))
        ...
        respostas["p6"] = st.radio("6) Interage com a equipe?", list(PESOS["p6"].keys()), horizontal=True, index=list(PESOS["p6"].keys()).index(st.session_state["respostas"].get("p6", list(PESOS["p6"].keys())[0])))
        respostas["j6"] = st.text_area("Justifique (Q6)", value=st.session_state["respostas"].get("j6", ""))

        progresso += 1
        st.progress(progresso / total_perguntas)

        respostas["p7"] = st.radio("7) É proativo?", list(PESOS["p7"].keys()), horizontal=True, index=list(PESOS["p7"].keys()).index(st.session_state["respostas"].get("p7", list(PESOS["p7"].keys())[0])))
        respostas["j7"] = st.text_area("Justifique (Q7)", value=st.session_state["respostas"].get("j7", ""))

        progresso += 1
        st.progress(progresso / total_perguntas)

        respostas["p8"] = st.radio("8) Gerencia bem o tempo/tarefas?", list(PESOS["p8"].keys()), horizontal=True, index=list(PESOS["p8"].keys()).index(st.session_state["respostas"].get("p8", list(PESOS["p8"].keys())[0])))
        ...
        respostas["p9"] = st.radio("9) Comunicação com equipe/áreas", list(PESOS["p9"].keys()), horizontal=True, index=list(PESOS["p9"].keys()).index(st.session_state["respostas"].get("p9", list(PESOS["p9"].keys())[0])))
        respostas["j9"] = st.text_area("Justifique (Q9)", value=st.session_state["respostas"].get("j9", ""))

        progresso += 1
        st.progress(progresso / total_perguntas)

        respostas["p10"] = st.radio("10) Contribui para resolução de conflitos?", list(PESOS["p10"].keys()), horizontal=True, index=list(PESOS["p10"].keys()).index(st.session_state["respostas"].get("p10", list(PESOS["p10"].keys())[0])))
        ...
        respostas["p11"] = st.radio("11) Contribuição para sucesso da empresa", list(PESOS["p11"].keys()), horizontal=True, index=list(PESOS["p11"].keys()).index(st.session_state["respostas"].get("p11", list(PESOS["p11"].keys())[0])))
